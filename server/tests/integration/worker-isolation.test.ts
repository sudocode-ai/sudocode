  ...
  () => {
    ...
    describe("worker lifecycle", () => {
      it("should transition worker through states correctly", async () => {
        ...
        // Wait for worker to complete and be cleaned up
        // Mock worker completes in ~200ms, but cleanup may take additional time
        const maxWait = 5000;
        const startTime = Date.now();

        while (
          pool.hasWorker(execution.id) &&
    ...
    describe("event forwarding", () => {
      it("should forward log events from worker to main process", async () => {
        ...
        await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Timeout waiting for log event")),
              10000
            )
          ),
        ]);
      ...
      it("should forward completion events from worker to main process", async () => {
        ...
        await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Timeout waiting for completion event")),
              10000
            )
          ),
        ]);
