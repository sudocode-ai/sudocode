...
describe("Projects API Routes", () => {
  ...
  describe("POST /api/projects/:projectId/close", () => {
    it("should return 404 when project is not open", async () => {
      const response = await request(app).post(
        "/api/projects/non-existent-id/close"
      );
  ...
  describe("POST /api/projects/init", () => {
    ...
    it("should successfully initialize new project", async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data).toHaveProperty("id");
      expect(response.body.data).toHaveProperty("path", newProjectPath);
      expect(response.body.data).toHaveProperty("openedAt");
      expect(response.body.data).toHaveProperty("hasWatcher");
    ...
    it("should set project name when provided", async () => {
      const newProjectPath = path.join(tempDir, "named-project");
      fs.mkdirSync(newProjectPath, { recursive: true });

      const response = await request(app)
        .post("/api/projects/init")
        .send({ path: newProjectPath, name: "My Custom Project" });

      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.data.name).toBe("My Custom Project");
    });

    it("should open already initialized project without reinitializing", async () => {
      // testProjectPath1 is already initialized in beforeEach
      const response = await request(app)
        .post("/api/projects/init")
        .send({ path: testProjectPath1 });
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data).toHaveProperty("id");
      expect(response.body.data.path).toBe(testProjectPath1);
    });

    it("should register initialized project in registry", async () => {
      const newProjectPath = path.join(tempDir, "registered-project");
      fs.mkdirSync(newProjectPath, { recursive: true });
    ...
    it("should track initialized project as open", async () => {
      const newProjectPath = path.join(tempDir, "open-project");
      fs.mkdirSync(newProjectPath, { recursive: true });
