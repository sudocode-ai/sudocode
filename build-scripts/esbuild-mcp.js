...
/**
 * esbuild config for sudocode-mcp
...
async function build() {
  console.log('Building sudocode-mcp bundle...');

  await esbuild.build({
    entryPoints: [path.join(rootDir, 'mcp/src/index.ts')],
    bundle: true,
    platform: 'node',
    target: 'node22',
    format: 'cjs',
    outfile: path.join(rootDir, 'dist/sea/mcp-bundle.js'),
  ...
  console.log('MCP bundle built: dist/sea/mcp-bundle.js');
}

build().catch(err => {
  console.error('MCP bundle failed:', err);
  process.exit(1);
});
