...
Section C: Unchanged`,
  ...
  describe('Scenario 6: Performance benchmarks', () => {
    it('should complete YAML conversion in under 10ms per entity', () => {
      const base: JSONLEntity[] = [];

      // Create a reasonably complex entity
      const local: JSONLEntity[] = [
        {
          id: 'i-perf',
          uuid: 'uuid-perf',
...
This is a multi-line description with various sections
to test YAML conversion performance.

## Section 1
Some content here that spans multiple lines and includes
various formatting elements.

## Section 2
More content with different text to ensure realistic
YAML conversion timing.

## Section 3
Final section with concluding remarks and additional
text to make this a realistic entity.`,
          status: 'open',
          priority: 1,
          created_at: '2025-01-01T00:00:00Z',
          updated_at: '2025-01-02T00:00:00Z',
          tags: ['performance', 'test', 'benchmark', 'yaml', 'conversion'],
          relationships: [
            {
              from: 'i-perf',
              from_type: 'issue',
      ...
      const worktree: JSONLEntity[] = [
        {
          id: 'i-perf',
          uuid: 'uuid-perf',
          title: 'Performance Test Entity Updated',
          description: local[0].description,
          status: 'in_progress',
          priority: 2,
          created_at: '2025-01-01T00:00:00Z',
          updated_at: '2025-01-03T00:00:00Z',
          tags: ['performance', 'test', 'updated'],
          relationships: local[0].relationships,
        },
      ];
      ...
      expect(merged).toHaveLength(1);
      expect(duration).toBeLessThan(10); // Less than 10ms for single entity

      // Log for debugging (won't show in normal test runs)
      if (duration >= 10) {
        console.warn(`YAML conversion took ${duration.toFixed(2)}ms (expected < 10ms)`);
      }
    });
    ...
    it('should handle bulk operations efficiently', () => {
      ...
      for (let i = 0; i < entityCount; i++) {
        const uuid = `uuid-bulk-${i}`;
        local.push({
          id: `i-local-${i}`,
          uuid,
          title: `Local Entity ${i}`,
      ...
      const duration = endTime - startTime;
      const avgPerEntity = duration / entityCount;

      expect(merged).toHaveLength(entityCount);
      expect(avgPerEntity).toBeLessThan(10); // Average < 10ms per entity

      // Log for debugging
      if (avgPerEntity >= 10) {
        console.warn(
          `Bulk merge averaged ${avgPerEntity.toFixed(2)}ms per entity (expected < 10ms)`
        );
      }
    });
  });
