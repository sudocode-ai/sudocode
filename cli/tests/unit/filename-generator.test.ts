...
describe('Filename Generator', () => {
  describe('titleToFilename', () => {
    it('should convert simple title to snake_case', () => {
      expect(titleToFilename('Implement Watch Mode')).toBe('implement_watch_mode');
    });

    it('should handle special characters', () => {
      expect(titleToFilename('User Auth & Permissions!')).toBe('user_auth_permissions');
    });

    it('should handle multiple spaces', () => {
      expect(titleToFilename('Multiple    Spaces    Test')).toBe('multiple_spaces_test');
    });

    it('should trim underscores from edges', () => {
      expect(titleToFilename('  Leading and Trailing  ')).toBe('leading_and_trailing');
    });
    ...
    it('should truncate long titles', () => {
      const longTitle = 'This is a very long title that should be truncated to a reasonable length for filenames';
  ...
  describe('generateUniqueFilename', () => {
    ...
    it('should use base filename when no collision', () => {
      const filename = generateUniqueFilename('Test Spec', 'spec-001', tempDir);
      expect(filename).toBe('test_spec.md');
    });

    it('should append ID when collision exists', () => {
      // Create existing file with different ID
      const existingFile = path.join(tempDir, 'test_spec.md');
...
Content`, 'utf8');
      ...
      const filename = generateUniqueFilename('Test Spec', 'spec-001', tempDir);
      expect(filename).toBe('test_spec_spec-001.md');
    });

    it('should reuse existing filename for same spec', () => {
...
Content`, 'utf8');
    ...
    it('should handle custom extension', () => {
      const filename = generateUniqueFilename('Test Spec', 'spec-001', tempDir, '.txt');
      expect(filename).toBe('test_spec.txt');
    });
  });
...
Content`, 'utf8');
      ...
      const found = findExistingSpecFile('spec-001', tempDir, 'Test Spec');
      expect(found).toBe(legacyFile);
    });

    it('should find title-based filename', () => {
...
Content`, 'utf8');
    ...
    it('should find title-based with ID suffix', () => {
      const titleWithIdFile = path.join(tempDir, 'test_spec_spec-001.md');
      fs.writeFileSync(titleWithIdFile, `---
id: spec-001
title: Test Spec
---
...
Content`, 'utf8');
    ...
    it('should return null when not found', () => {
      const found = findExistingSpecFile('spec-404', tempDir, 'Nonexistent');
