...
describe("Git Merge-File Wrapper", () => {
  describe("mergeYamlContent (async)", () => {
    it("should perform clean merge with no conflicts", async () => {
      ...
      const result: MergeResult = await mergeYamlContent(input);
    ...
    it("should detect conflicts when both sides modify the same line", async () => {
      const input: MergeInput = {
        base: "title: Original\nstatus: open",
    ...
    it("should handle merge with additions from both sides", async () => {
    ...
    it("should handle merge when only our side changed", async () => {
      const input: MergeInput = {
        base: "title: Original\nstatus: open",
        ours: "title: Updated\nstatus: in_progress",
        theirs: "title: Original\nstatus: open",
    ...
    it("should handle merge when only their side changed", async () => {
      const input: MergeInput = {
        base: "title: Original\nstatus: open",
        ours: "title: Original\nstatus: open",
    ...
    it("should handle multi-line YAML content", async () => {
      const input: MergeInput = {
        base: "title: Original\ndescription: |\n  Line 1\n  Line 2\nstatus: open",
        ours: "title: Updated\ndescription: |\n  Line 1\n  Line 2\nstatus: open",
        theirs: "title: Original\ndescription: |\n  Line 1\n  Line 2\nstatus: closed",
    ...
    it("should handle empty base (both sides added)", async () => {
      const input: MergeInput = {
        base: "",
    ...
    it("should handle large YAML content", async () => {
  ...
  describe("mergeYamlContentSync (synchronous)", () => {
    it("should perform clean merge with no conflicts", () => {
      // Same pattern as async: non-overlapping changes
      const input: MergeInput = {
        base: "field1: base\n\nfield3: base",
        ours: "field1: ours\n\nfield3: base",
        theirs: "field1: base\n\nfield3: theirs",
      ...
      const result: MergeResult = mergeYamlContentSync(input);
    ...
    it("should handle merge with additions from both sides", () => {
      ...
      // Same as async version - both adding at end creates conflict
      expect(result.success).toBe(false);
      expect(result.hasConflicts).toBe(true);
      expect(result.content).toContain("title: Original");
      expect(result.content).toContain("ours_field: added by us");
    ...
    it("should produce same result as async version for clean merge", async () => {
      const input: MergeInput = {
        base: "field1: base\n\nfield3: base",
        ours: "field1: ours\n\nfield3: base",
        theirs: "field1: base\n\nfield3: theirs",
      ...
      const asyncResult = await mergeYamlContent(input);
      const syncResult = mergeYamlContentSync(input);

      expect(syncResult.success).toBe(asyncResult.success);
      expect(syncResult.hasConflicts).toBe(asyncResult.hasConflicts);
      // Both should produce clean merges
      expect(syncResult.success).toBe(true);
      expect(asyncResult.success).toBe(true);
    });

    it("should produce same result as async version for conflict", async () => {
      const input: MergeInput = {
        base: "title: Base",
        ours: "title: Ours",
        theirs: "title: Theirs",
  ...
  describe("Real-world Scenarios", () => {
    it("should handle typical issue JSONL merge scenario", async () => {
...
updated_at: "2024-01-02T11:00:00Z"`;
    ...
    it("should handle spec with complex nested YAML", async () => {
