...
export async function handleStats(
  ...
  const relResults = relationshipsStmt.all() as Array<{
  ...
  const totalRelationships = relResults.reduce(
  ...
  const recentSpecs = allSpecs.filter(
    (s) => new Date(s.updated_at) > sevenDaysAgo
  ).length;
  ...
  const closedIssues = allIssues.filter(
    (i) => i.closed_at && new Date(i.closed_at) > sevenDaysAgo
  ).length;
  ...
  const relTypes = Object.entries(relationshipCounts)
